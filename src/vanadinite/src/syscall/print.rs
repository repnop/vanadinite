...
pub fn print(virt: VirtualAddress, len: usize) {
    log::info!("Attempting to print memory at {:#p} (len={})", virt, len);
    ...
    if virt.is_kernel_region() {
        log::info!("Process tried to get us to write to our own memory >:(");
        Scheduler::mark_active_dead(&*SCHEDULER);
        return;
    } else if !valid_memory {
        log::info!("Process tried to get us to write to unmapped memory >:(");
        Scheduler::mark_active_dead(&*SCHEDULER);
        return;
    }
